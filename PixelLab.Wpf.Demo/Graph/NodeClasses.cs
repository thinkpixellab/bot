  ...
  public class NodeCollection<TNode> where TNode : IEquatable<TNode> {
    public NodeCollection(IEnumerable<TNode> nodes) {
      ...
      Util.RequireArgument(
          m_nodeValues.All(item => item != null),
          "nodes",
          "All items must be non-null.");

      Util.RequireArgument(m_nodeValues.AllUnique(), "nodes");
    ...
    public Node<TNode> this[TNode value] {
      get {
        if (!m_nodes.ContainsKey(value)) {
          Util.RequireArgument(m_nodeConnections.ContainsKey(value), "value", "No node exists with the provided value.");
    ...
    public void Add(TNode node) {
      Util.RequireArgument(!m_nodeValues.Contains(node), "node", "Already have this value.");
    ...
    public void RemoveEdge(TNode node1, TNode node2) {
      Util.RequireArgument(!node1.Equals(node2), "node2", "One cannot create an edge between the same node.");
    ...
    public void MoreFriends(TNode node) {
      Util.RequireArgument(m_nodeConnections.ContainsKey(node), "node");
    ...
    public void Churn() {
      Util.Require(m_nodeConnections.Count >= 3);
      ...
      if (currentRatio <= c_idealConnectionRatio || m_nodeConnections[item].Count < c_minConnections) {
        ...
        Util.Require(m_nodeConnections[item].Count != (m_nodeConnections.Count - 1));
        ...
        Util.Require(m_nodeConnections[item].Count > 0);
